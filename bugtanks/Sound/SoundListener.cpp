...
bool SoundListener::handleEvent(const logic::EventData &mEvent)
{
	...
	if(logic::EvtData_To_ActorStartMove::msEventType == mEvent.getEventType()) 
	{
		...
		{
			//TODO: change soundID
			mpSoundSystem->playSound(sound::Sound_PlayerTankLongRangeMovementMoving, logic::ActorHandler::getInstance()->getActor(evtData.mId)->getKey(), 0); 
		}
	}
	...
	if(logic::EvtData_To_ActorStopMove::msEventType == mEvent.getEventType()) 
	{
		...
		if(logic::Actor::ActorType_Tank == logic::ActorHandler::getInstance()->getActor(evtData.mId)->getActorType())
		{
			mpSoundSystem->SoundOff(logic::ActorHandler::getInstance()->getActor(evtData.mId)->getKey(), 0);
		}
	}*/
	...
	if(logic::EvtData_From_ActorCreated::msEventType == mEvent.getEventType()) 
	{
		...
		{
			// HACK - Test the dynamic cast for NULL
	...
	//Plays a sound where the pActor died and then stops its channels
	...
	else if( logic::EvtData_From_WeaponFired::msEventType == mEvent.getEventType() ) 
	{
		...
		if( pActor != NULL && pActor->getActorType() == logic::Actor::ActorType_Tank ) 
		{
			...
			if(pWeapon->getWeaponType() == logic::Weapon::WeaponTypes_Minigun) 
			{
				...
				if( mpSoundSystem->mChannels[mpSoundSystem->findActor(pActor->getKey())].mFire == true )
				{
					mpSoundSystem->playSound(sound::Sound_WeaponMinigunFire, pActor->getKey(), Channel_Attack);
				}
				else
				{
					mpSoundSystem->SoundOff(pActor->getKey(), Channel_Attack);
					mpSoundSystem->playSound(sound::Sound_WeaponMinigunEnd, pActor->getKey(), Channel_Attack);
				}
			}*/
			...
			{
				if(!mpSoundSystem->isPlaying(pActor->getKey(), Channel_Attack))
				{
					mpSoundSystem->playSound(sound::Sound_WeaponFlamethrowerFire, pActor->getKey(), Channel_Attack);
				}
			}
	...
	if(logic::EvtData_To_ActorStartRotate::msEventType == mEvent.getEventType()) //TODO: check for more actors
	{
		...
		mpSoundSystem->playSound(sound::Sound_PlayerTankLongRangeMovementMoving, logic::ActorHandler::getInstance()->getActor(logic::PlayerInfo::getInstance()->getPlayerId())->getKey(), Channel_Rotation); 
	...
	if(logic::EvtData_To_ActorStopRotate::msEventType == mEvent.getEventType()) //TODO: check for more actors
	{
		...
		//if(logic::Actor::ActorType_Tank == logic::ActorHandler::getInstance()->getActor(evtData.mId)->getActorType())
		//{
			mpSoundSystem->SoundOff(logic::ActorHandler::getInstance()->getActor(evtData.mId)->getKey(), Channel_Rotation);
		//}
	...
	else if( logic::EvtData_From_EnemyHit::msEventType == mEvent.getEventType() ) 
	{
		...
		assert(pShooter != NULL);
		assert(pWeapon != NULL);
		assert(pEnemy != NULL);

		utilities::Position pos = pEnemy->getPosition();
	...
	else if( logic::EvtData_From_ActorCollisionObject::msEventType == mEvent.getEventType() ) 
	{
		...
		if( pFirstActor != NULL && pSecondActor != NULL )
		{
			...
			else if( pFirstActor->getActorType() == logic::Actor::ActorType_Munition ) 
			{
				...
				//{
				//	mpSoundSystem->playSound(sound::Sound_WeaponMinigunImpact, pSecondActor->getKey(), Channel_Attack);
				//}
				...
				{
					mpSoundSystem->playSound(sound::Sound_WeaponCannonImpact, pFirstActor->getKey(), Channel_Attack);
				}
				else if( weapon == logic::Weapon::WeaponTypes_GaussCannon )
				{
					mpSoundSystem->playSound(sound::Sound_WeaponGausscannonImpact, pSecondActor->getKey(), Channel_Attack);
				}
