...
int SoundSystem::findActor( logic::KeyType actorID )
{
	...
	ERROR_MESSAGE( "Sound Error: 60003 - No pActor found with the provided pActor ID\n");
	return -1;
}

void SoundSystem::updateChannel( const ActorInfo& actorInfo )
{
	logic::Actor* pActor = mpActorHandler->getActor(actorInfo.mActorID);
	if( pActor->getActorType() != logic::Actor::ActorType_Munition )
	{
		...
		if( iActorID != -1 )
		{
			...
			*/
			...
			if(actorInfo.mMoving != mChannels[iActorID].mMoving)
			{
				...
				{
					if( pActor->getActorType() == logic::Actor::ActorType_Tank)
					{
						if( dynamic_cast<logic::Tank*>(pActor)->getTankType() == logic::Tank::TankType_LongRange ){
						...
						else if( dynamic_cast<logic::Tank*>(pActor)->getTankType() == logic::Tank::TankType_CloseCombat ){
			...
			if( pActor->getActorType() == logic::Actor::ActorType_Tank && actorInfo.mFire != mChannels[iActorID].mFire )
			{
				if( dynamic_cast<logic::Tank*>(pActor)->getWeapon()->getWeaponType() == logic::Weapon::WeaponTypes_Minigun )
				{
					if( actorInfo.mFire )
					{
				...
				else if ( dynamic_cast<logic::Tank*>(pActor)->getWeapon()->getWeaponType() == logic::Weapon::WeaponTypes_Flamethrower )
				{
					if( actorInfo.mFire )
					{
...
int SoundSystem::findMovementSound( logic::KeyType actorID )
{
	...
	else if (pActor->getActorType() == logic::Driver::ActorType_Driver)
	{
