...
HRESULT DX10Module::init(long width, long height)
{
	...
	mLight->MatrixOrthoLH(700.0f, 700.0f, 1.0f, 800.0f);
	...
	mpTextureHandler->addTexture("../../Textures/Ground/tile04.png",						"WorldPlane01");
	...
	mpTextureHandler->addTexture("../../Textures/soil04.jpg",								"WorldPlane04");
	...
	// List
	mpTextureHandler->addTexture("../../Textures/soil04.jpg",								"ListBG");
...
void DX10Module::update(float deltaTime)
{
	...
	Particle* pParticle1 = mParticleHandler.getParticleSystem("SmallExplosion");
	Particle* pParticle2 = mParticleHandler.getParticleSystem("BigExplosion");
	//pParticle2 = NULL;

	if(pParticle1 != NULL && GetAsyncKeyState('R') & 0x8000)
	{
		if(mButtonPressTimer > 0.2f)
		{
			mButtonPressTimer = 0.0f;
			pParticle1->addEmitter(D3DXVECTOR3(0.0f, 20.0f, 0.0f));
		}
	}
	if(pParticle1 != NULL && GetAsyncKeyState('T') & 0x8000)
	{
		if(mButtonPressTimer > 0.2f)
		{
			mButtonPressTimer = 0.0f;
			pParticle2->addEmitter(D3DXVECTOR3(50.0f, 20.0f, 0.0f));
		}
	}
...
void DX10Module::render()
{
	...
	mShaderHandler.getEffect("MainShader")->getEffect()->GetTechniqueByIndex(0)->GetPassByIndex(ShaderPasses::CEL)->Apply(0);
	mShaderHandler.getEffect("MainShader")->getEffect()->GetTechniqueByIndex(0)->GetPassByIndex(ShaderPasses::MULTITEX)->Apply(0);
...
HRESULT DX10Module::loadShaders()
{
	...
	mShaderHandler.getEffect("MainShader")->createInputLayout(0, ShaderPasses::GUI, VertexTypes::VERTEX_GUI);
	mShaderHandler.getEffect("MainShader")->createInputLayout(0, ShaderPasses::CEL, VertexTypes::VERTEX_TRANSFORM);
	mShaderHandler.getEffect("MainShader")->createInputLayout(0, ShaderPasses::ANIMATION, VertexTypes::VERTEX_ANIMATION);
	mShaderHandler.getEffect("MainShader")->createInputLayout(0, ShaderPasses::MULTITEX, VertexTypes::VERTEX_MULTI_TEXTURE);
	mShaderHandler.getEffect("MainShader")->createInputLayout(0, ShaderPasses::PARTICLE, VertexTypes::VERTEX_PARTICLES);
