...
* @section DESCRIPTION
*
* Handles short-lived particles particles
*/
...
ParticleEphemeral::ParticleEphemeral(ID3D10Device *pDevice, Effect *pEffect, char* name, int passIndex, int layoutIndex, int initParticles, int maxChildParticles, int offset) 
...
ParticleEphemeral::~ParticleEphemeral()
{
}

void ParticleEphemeral::updateParticleStorage(ID3D10Resource* pParticleStorage)
{
	// Check if an update is required for particles and if the system has been initialized
	if(mFlagUpdate)
	{
		DEBUG_MESSAGE(LEVEL_HIGHEST, "mFlagUpdate = true;");
