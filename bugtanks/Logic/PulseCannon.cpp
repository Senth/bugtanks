...
void PulseCannon::fireWeapon(KeyType ownerId)
{
    ...
    if(mCooldownCounter > Weapon::getCooldown())
    {
	   ...
	   if(mBullets <= 1)
	   {
		   safeQueueEvent(EventDataPtr(myNew EvtData_To_ActorCreateMunition(Munitions::MunitionType_Pulse, position, direction, ownerId)));
	   }	 
	   ...
	   if(mBulletKeys[mBulletCounter] != INVALID_KEY)
	   {
		   ...
		   //mpDX10Module->mParticleHandler.getParticleSystem("Sparks")->addEmitter(position.convertToD3DXVECTOR3());
