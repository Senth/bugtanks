...
void ActorHandler::update(float deltaTime)
{
	...
	for(actorIt = mDynamicActorMap.begin() ; actorIt != mDynamicActorMap.end(); ++actorIt)
	{
		...
		if(update)
		{
			...
			else if(getActor(actorInfo.mActorID)->getActorType() == Actor::ActorType_Munition && getActor(actorInfo.mActorID)->getVisible())
			{
				...
				{
					Actor *pActor = getActor(pMunition->getOwner());
					if(pActor != NULL && (pActor->getActorSubType() == Enemy::EnemyType_FireAnt || pActor->getActorSubType() == Enemy::EnemyType_BlackWidow))
					{	
						mpLogicQuadTree->collisionMunition(actorInfo.mActorID);
					}
					else
