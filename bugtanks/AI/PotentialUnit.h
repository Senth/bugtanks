...
public:
	...
	* @param applyOnMapPotentialField if the unit's potential field should be added to the map's potential field.
	*/
	PotentialUnit(logic::Actor* pActor, bool applyOnMapPotentialField);
	...
	*/
	...
	/**
	* Returns the last map position
	* @return the last map position
	*/
	inline const utilities::MapCoordinate& getLastMapPosition() const
	{
		return mCurrentPos;
	}

	/**
	* Returns the second last (penultimate) position
	* @returns the position before we update the last time, i.e. the penultimate position
	*/
	inline const utilities::MapCoordinate& getPenultimateMapPosition() const
	{
		return mPreviousPos;
	}
	...
	*/
	inline bool applyOnMapPotentialField() const {return mApplyOnMapPotentialField;}
...
private:
	...
	*/
	...
	bool mRotated;
	bool mApplyOnMapPotentialField;
	bool mActive;
