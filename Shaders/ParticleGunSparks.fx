					...
					inout PointStream<PARTICLE_INPUT> particleStream)
{
	...
	{
		float3 pos		= ParticleStorage.Load(int4(0, gIn[0].ID, GUNSPARKS, 0)).rgb;
		float3 vel		= ParticleStorage.Load(int4(1, gIn[0].ID, GUNSPARKS, 0)).rgb;
		float3 misc		= ParticleStorage.Load(int4(2, gIn[0].ID, GUNSPARKS, 0)).rgb;
	...
	else if(gIn[0].Type == ACTIVE_EMITTER)
	{
		if(gIn[0].Age > 0.1f)
		{
			PARTICLE_INPUT p;
			p.PosH		= gIn[0].PosH;
			p.Velocity	= gIn[0].Velocity * 50.0f;
			p.Size		= gIn[0].Size;
			p.Age		= 0.01f;
			p.Type		= CHILD;
			p.ID		= -1;
			...
			p.PosH		= gIn[0].PosH;
			p.Velocity	= gIn[0].Velocity * 25.0f;
			p.Size		= gIn[0].Size;
			p.Age		= 0.01f;
			p.Type		= CHILD;
			p.ID		= -1;
			...
			p.PosH		= gIn[0].PosH;
			p.Velocity	= gIn[0].Velocity * 50.0f + float3(10.0f, 0.0f, 0.0f);
			p.Size		= gIn[0].Size;
			p.Age		= 0.01f;
			p.Type		= CHILD;
			p.ID		= -1;
			...
			p.PosH		= gIn[0].PosH;
			p.Velocity	= gIn[0].Velocity * 50.0f + float3(5.0f, 0.0f, 0.0f);
			p.Size		= gIn[0].Size;
			p.Age		= 0.01f;
			p.Type		= CHILD;
			p.ID		= -1;
			...
			p.PosH		= gIn[0].PosH;
			p.Velocity	= gIn[0].Velocity * 25.0f + float3(5.0f, 0.0f, 0.0f);
			p.Size		= gIn[0].Size;
			p.Age		= 0.01f;
			p.Type		= CHILD;
			p.ID		= -1;
...
[maxvertexcount(4)]
void GS_PARTICLE_SPARKS_DRAW(point GS_PARTICLE_INPUT gIn[1],
...
float4 PS_PARTICLE_SPARKS_DRAW(PS_PARTICLE_INPUT input) : SV_Target
{
	return input.Color;
}
