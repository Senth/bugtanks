#ifndef PARTICLE_TANK_EXPLOSION_FX
#define PARTICLE_TANK_EXPLOSION_FX
 ...
 */
[maxvertexcount(93)]
void GS_PARTICLE_TANK_EXPLOSION_SO(point PARTICLE_INPUT gIn[1],
								   inout PointStream<PARTICLE_INPUT> particleStream)
{
	...
	{
		float3 pos		= ParticleStorage.Load(int4(0, gIn[0].ID, TANKEXPLO, 0)).rgb;
		float3 vel		= ParticleStorage.Load(int4(1, gIn[0].ID, TANKEXPLO, 0)).rgb;
		float3 misc		= ParticleStorage.Load(int4(2, gIn[0].ID, TANKEXPLO, 0)).rgb;
	...
	else if(gIn[0].Type == ACTIVE_EMITTER)
	{
		if(gIn[0].Age > 0.1f)
		{
			...
			for(i=0; i<20; i++)
			{
				...
				PARTICLE_INPUT p;
				p.PosH		= gIn[0].PosH;
				p.Velocity	= float3(25.0f * sin(angle), 0.0f, 25.0f * cos(angle));
				p.Size		= float2(9.0f, 9.0f);
				p.Age		= 0.01f;
				p.Type		= CHILD;
				p.ID		= -1;
				particleStream.Append(p);
			...
			for(i=0; i<16; i++)
			{
				...
				PARTICLE_INPUT p;
				p.PosH		= gIn[0].PosH;
				p.Velocity	= float3(20.0f * sin(angle), 12.50f, 20.0f * cos(angle));
				p.Size.x	= 9;//gIn[0].Size;
				p.Size.y	= 9;
				p.Age		= 0.01f;
				p.Type		= CHILD;
				p.ID		= -1;
			...
			*/
			...
			for(i=0; i<10; i++)
			{
				...
				PARTICLE_INPUT p;
				p.PosH		= gIn[0].PosH;
				p.Velocity	= float3(10.0f * sin(angle), 20.0f, 10.0f * cos(angle));
				p.Size.x	= 9;//gIn[0].Size;
				p.Size.y	= 9;
				p.Age		= 0.01f;
				p.Type		= CHILD;
 ...
 * Particle drawing
 */
GS_PARTICLE_INPUT VS_PARTICLE_TANK_EXPLOSION_DRAW(PARTICLE_INPUT input)
{
	GS_PARTICLE_INPUT output = (GS_PARTICLE_INPUT)0;

	output.PosH		= float4(input.PosH, 1.0f);
	output.Size		= input.Size;
	output.Color	= float4(0.0f, 0.0f, 0.0f, 0.3f);
	output.Type		= input.Type;
	/*
	if(output.Size.x == 9)
	{
		output.Color = float4(1.0f, 0.7f, 0.0f, 0.8f);
	}
*/
	return output;
}
...
[maxvertexcount(4)]
void GS_PARTICLE_TANK_EXPLOSION_DRAW(point GS_PARTICLE_INPUT gIn[1],
...
float4 PS_PARTICLE_TANK_EXPLOSION_DRAW(PS_PARTICLE_INPUT input) : SV_Target
{
	return input.Color;
}
