#ifndef PARTICLE_SMALL_EXPLOSION_FX
#define PARTICLE_SMALL_EXPLOSION_FX
 ...
 */
[maxvertexcount(7)]
void GS_PARTICLE_SMALL_EXPLOSION_SO(point PARTICLE_INPUT gIn[1],
					inout PointStream<PARTICLE_INPUT> particleStream)
{
	...
	{
		float3 pos		= ParticleStorage.Load(int4(0, gIn[0].ID, SMALLEXPLO, 0)).rgb;
		float3 vel		= ParticleStorage.Load(int4(1, gIn[0].ID, SMALLEXPLO, 0)).rgb;
		float3 misc		= ParticleStorage.Load(int4(2, gIn[0].ID, SMALLEXPLO, 0)).rgb;
 ...
 * Particle drawing
 */
GS_PARTICLE_INPUT VS_PARTICLE_SMALL_EXPLOSION_DRAW(PARTICLE_INPUT input)
{
...
[maxvertexcount(4)]
void GS_PARTICLE_SMALL_EXPLOSION_DRAW(point GS_PARTICLE_INPUT gIn[1],
...
float4 PS_PARTICLE_SMALL_EXPLOSION_DRAW(PS_PARTICLE_INPUT input) : SV_Target
{
	return input.Color;
}
